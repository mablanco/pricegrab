...
- [ ] T034 [P] [US1] Locale-aware parsing (comma decimal separator in `es-ES`) is covered at the JVM-unit-test level by `LocaleNumberFormatterTest` and `OfferParserTest`. The originally-planned `CompareScreenLocaleEsTest` instrumented test was dropped: `Locale.setDefault` from `@BeforeClass` does not reliably propagate into the target app's process on the AOSP ATD image, and per-app locale APIs (`LocaleManager.setApplicationLocales`) are Android 13+ only while `minSdk` is 24. Revisit when Android 13 becomes the minimum supported version.
...
- [x] T052 [P] Added `fastlane/metadata/android/en-US/{title,short_description,full_description}.txt` plus `changelogs/1.txt` (initial release notes). Originally landed in PR D under `android/fastlane/...`; moved to the repo-root `fastlane/...` in PR `chore/009-fdroid-prep` to match the path F-Droid auto-discovers.
- [x] T053 [P] Added the `fastlane/metadata/android/es-ES/` mirror with translated title, short and full descriptions, and the matching `changelogs/1.txt`. Both locales respect F-Droid's 50/80/4000/500-character limits. Same path migration as T052.

...
Modified principles: N/A (initial ratification; replaced placeholders [PRINCIPLE_1_NAME..5])
  - I.   Modern Mobile UX (new)
  - II.  Accessibility (NON-NEGOTIABLE) (new)
  - III. Internationalization (ES/EN) (new)
  - IV.  Offline-First Performance & Reliability (new)
  - V.   Test-First Quality (NON-NEGOTIABLE) (new)
Added sections:
  - Privacy & Platform Constraints (replaces [SECTION_2_NAME])
...
Templates requiring updates:
  ...
  - README.md                               ⚠ pending (project README does not yet exist;
                                              create when project scaffolding lands)
Follow-up TODOs:
  - TODO(PROJECT_SCAFFOLD): Generate the Android project (Kotlin + Jetpack Compose or chosen
    stack) and add a README.md that links to this constitution.
-->
...
- **Minimal permissions**: The app MUST request the smallest possible set of Android
  ...
  `INTERNET` permission.
- **Target the latest stable Android SDK** for `targetSdk` and stay within Google Play's
  current target-API requirement window. `minSdk` SHOULD cover ≥ 95% of active devices and
  MUST be justified in the plan when raised.
